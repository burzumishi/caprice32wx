...
{
    wxCHECK_MSG(combo, false, "Invalid combobox in wxSTEPrependComboBoxString");
...
{
    wxCHECK_RET(combo, "Invalid combobox in wxSTEInitComboBoxStrings");
...
{
    wxCHECK_RET(menu, "Invalid wxMenu in wxSTEInitMenuStrings");
...
{
    return wxString::Format("%s|%d|%d|%d|%d>",
...
bool wxSTEditorFoundStringData::FromString(const wxString& findAllString)
{
    ...
    m_fileName = s.BeforeFirst('|');
    s = s.AfterFirst('|');

    if (s.BeforeFirst('|').ToLong(&value))
    {
...
bool wxSTEditorFindReplaceData::GotoFindAllString(const wxSTEditorFoundStringData& foundStringData,
                                                  wxSTEditor* editor)
{
    wxCHECK_MSG(editor, false, "Invalid wxSTEditor to goto line in.");
                                           ...
                                           const wxString &configPath)
{
    ...
    if (config.Read(key + "/FindFlags", &val))
    {
        SetFlags(int(val));
        return true;
    }
...
{
    wxString key(wxSTEditorOptions::FixConfigPath(configPath, false));
    config.Write(key + "/FindFlags", GetFlags());
}
                                         ...
                                         long style, const wxString& name)
{
    ...
    //SetMarginWidth(STE_MARGIN_NUMBER, TextWidth(wxSTC_STYLE_LINENUMBER, "_9999"));
...
void wxSTEditorFindResultsEditor::SetResults(const wxSTEditorFindReplaceData& findReplaceData)
{
    ...
    for (n = 0; n < count; n++)
    {
        ...
        wxString lineString(wxString::Format("%5d", foundStringArray[n].GetLineNumber()+1));
        AppendText(lineString);
        StartStyling(pos, 31);
        SetStyling(lineString.Length(), 4);

        pos = GetLength();
        AppendText(" : " + foundStringArray[n].GetLineString());
...
{
    wxCHECK_RET(data, "Invalid find replace data in wxSTEditorFindReplaceDialog::SetData");
...
void wxSTEditorFindReplacePanel::OnMenu(wxCommandEvent& event)
{
    ...
    switch (event.GetId())
    {
        ...
        case ID_STEDLG_INSERTMENURE_ANYCHAR   : c = "."; break;
        case ID_STEDLG_INSERTMENURE_RANGE     : c = "[]";  ipos = -1; break;
        case ID_STEDLG_INSERTMENURE_NOTRANGE  : c = "[^]"; ipos = -1; break;
        case ID_STEDLG_INSERTMENURE_BEGINLINE : c = "^"; break;
        case ID_STEDLG_INSERTMENURE_ENDLINE   : c = "$"; break;
        ...
        case ID_STEDLG_INSERTMENURE_0MATCHES  : c = "*"; break;
        case ID_STEDLG_INSERTMENURE_1MATCHES  : c = "+"; break;
        case ID_STEDLG_INSERTMENURE_01MATCHES : c = "?"; break;

        case ID_STEDLG_INSERTMENURE_ALPHANUM  : c = "[a-zA-Z0-9]"; break;
        case ID_STEDLG_INSERTMENURE_ALPHA     : c = "[a-zA-Z]"; break;
        case ID_STEDLG_INSERTMENURE_NUMERIC   : c = "[0-9]"; break;
        case ID_STEDLG_INSERTMENURE_TAB       : c = "\\t"; break;
        default : break;
    }
...
//-----------------------------------------------------------------------------
const wxString wxSTEditorFindReplaceDialogNameStr = "wxSTEditorFindReplaceDialogNameStr";
...
*/
    ...
    //wxPrintf("wxSTEditorFindReplaceDialog::OnSize %d %d   %d %d\n", GetSize().x, GetSize().y, event.GetSize().x, event.GetSize().y);
}
