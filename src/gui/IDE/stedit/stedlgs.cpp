...
{
    wxCHECK_RET(editorPrefData.GetPrefs().IsOk(), "Invalid preferences");
...
void wxSTEditorPrefDialogPagePrefs::GetControlValues()
{
    ...
    for (n = 0; n < count; n++)
    {
        ...
        if ((win_id == ID_STEDLG_FOLD_STYLES_LISTBOX) && wxDynamicCast(win, wxCheckListBox))
        {
            ...
            wxFAIL_MSG("Unknown control type in wxSTEditorPrefDialogPagePrefs::GetControlValues");
    }
}
...
void wxSTEditorPrefDialogPagePrefs::SetControlValues()
{
    ...
    for (n = 0; n < count; n++)
    {
        ...
        if ((win_id == ID_STEDLG_FOLD_STYLES_LISTBOX) && wxDynamicCast(win, wxCheckListBox))
        {
            ...
            wxFAIL_MSG("Unknown control type in wxSTEditorPrefDialogPagePrefs::SetControlValues");
    }
}
...
wxString wxSTEditorPrefDialogPageStyles::sm_helpString(
    "Help On Setting Styles\n\n"

    "This editor supports syntax highlighting for a number of different programming languages. "
    "In order to make it easy to create your own color and style theme, a single generic list of styles is used. "
    "This is because some languages have rather esoteric idioms and if this editor catered to each and every one there would be hundreds of different styles to have to adjust. "
    "The list tries to encompass most of the common styles for all the language lexers (code parsers). "
    "Therefore, the names given to the styles are meant only as hints as to what the style will be used for and you can map any editor style to any lexer style. "
    "For more information you should examine the \"Languages\" tab where you can change the mapping of the editor and lexer styles.\n\n"

    "Select the style you want to change by clicking on the margin or double clicking the line. "
    "You can change the individual attributes for a style by checking the checkboxes to the left or "
    "uncheck them to use the attribute value from the \"Default Style\" in the \"Lexer Styles\". "
    "It is a good idea to set the \"Default Style\" first so that you only set the values for the other styles that you want to be different. "
    "Attributes that do not apply for specific styles are disabled.\n\n"

    "The \"Editor Colors\" page allows you to change the overall appearance of the editor for any language.\n\n"

    "The \"Lexer Styles\" page shows a list of styles to allow you to adjust them all at once to easily create a theme that'll look good for any language. "
    "You can also select particular languages to see what styles they use and get a feel for what they would look like. "
    "The styles that are used for the currently set lexer in the \"Languages\" tab are shown with a white cross in the margin."
);
                               ...
                               :wxSTEditorPrefDialogPageBase(editorPrefData, prefData, parent, winid)
{
    ...
    wxCHECK_RET(steStyles.IsOk(), "Invalid styles"); // langs may be NULL
    ...
    for (n = 0; n < count; n++)
        m_fontChoice->Append("*" + faceNames.Item(n));
...
void wxSTEditorPrefDialogPageStyles::FillStyleEditor(wxSTEditor* editor)
{
    ...
    for (n = 0; n < count; n++)
    {
        ...
        if (langStyles[n] < STE_STYLE_LANG__MAX)
        {
            ...
            {
                text += wxString(' ', m_style_max_len - text.Len());
...
void wxSTEditorPrefDialogPageStyles::SetupEditor(wxSTEditor* editor)
{
    ...
    editor->SetMarginWidth(STE_MARGIN_NUMBER, editor->TextWidth(wxSTC_STYLE_LINENUMBER, "_199"));
...
void wxSTEditorPrefDialogPageStyles::OnEvent(wxCommandEvent &event)
{
    ...
    switch (event.GetId())
    {
        ...
        {
            steStyles.SetFaceName(m_current_style, m_fontChoice->GetStringSelection().AfterLast('*'));
            break;
        }
        // Font size  ---------------------------------------------------------
...
void wxSTEditorPrefDialogPageStyles::SetControlValues()
{
    ...
    if (use_font)
    {
        ...
        if (font_sel == wxNOT_FOUND)
            font_sel = m_fontChoice->FindString("*"+steStyles.GetFaceName(m_current_style));
...
wxString wxSTEditorPrefDialogPageLangs::sm_helpString(
    "Help On Language Lexers (code parsers)\n\n"

    "This editor supports syntax highlighting for a number of different programming languages. "
    "Choose the language lexer you want to use for the current document using the \"Language\" choice control. "
    "The file patterns are a semicolon ';' separated list of file patterns to detect what language to use for different file types. "
    "For example; C/C++ has the patterns '*.c;*.cpp;*.h' which means that files ending with .c, .cpp, .h will be lexed using C/C++ programming language.\n\n"

    "The \"Styles\" tab allows you to change the mapping between the editor styles on the left and the lexer styles on the right. "
    "See the overall \"Styles\" tab to adjust the look of the editor styles and to learn about the names given to the editor styles. "
    "Use the \"Choose style\" choice control to select a different editor style, note that the names of the editor styles are just hints for what they're most used for and can be arbitrarily mapped to any lexer style. "
    "Select a different lexer style to modify by clicking on the margin or double clicking on the appropriate line.\n\n"

    "The \"Keywords\" tab allows you to view the special keywords for the language. "
    "Different languages may have different numbers of keyword sets and each one can be given their own styling. "
    "You may add your own additional keywords by entering them as a space separated list into the \"Additional keywords\" text control. "
);
...
wxSTEditorPrefDialogPageLangs::wxSTEditorPrefDialogPageLangs(const wxSTEditorPrefPageData& editorPrefData,
                              ...
                              :wxSTEditorPrefDialogPageBase(editorPrefData, prefData, parent, winid)
{
    wxCHECK_RET(GetPrefData().GetLangs().IsOk(), "Invalid languages");
    wxCHECK_RET(GetPrefData().GetStyles().IsOk(), "Invalid styles");
...
void wxSTEditorPrefDialogPageLangs::SetControlValues()
{
    ...
    for (n = 0; n < style_count; n++)
    {
        ...
        if (ste_style >= 0)
        {
            ...
            styleName += wxString(' ', wxMax(1UL, m_max_stylename_length - styleName.Length()));
            styleName += steLangs.GetStyleDescription(m_current_lang, n);

            m_styleEditor->AppendText(styleName + "\n");
        }
    }
    ...
    for (n = 0; n < keywords_count; n++)
        m_keywordsChoice->Append(wxString::Format("%d", (int)(n+1)));
                                   ...
                                   long style, const wxString& name )
{
    ...
    wxCHECK_MSG(m_editorPrefData.GetPrefs().IsOk() ||
                ...
                m_editorPrefData.GetLangs().IsOk(), false, "At least one page must be added.");
                                        ...
                                        long style)
{
    ...
    SET_STATTEXT(ID_STEPROP_NUMLINES_TEXT, wxString::Format("%d", m_editor->GetLineCount()));
    SET_STATTEXT(ID_STEPROP_NUMCHARS_TEXT, wxString::Format("%d", m_editor->GetTextLength()));
    SET_STATTEXT(ID_STEPROP_NUMWORDS_TEXT, wxString::Format("%d", (int)m_editor->GetWordCount()));
    ...
    SET_STATTEXT(ID_STEPROP_NUMTABS_TEXT, wxString::Format("%d", tabs));

    wxString eolStr;
    if (crlf > 0)
        eolStr += wxString::Format("CRLF (DOS/Win)=%d", crlf);
    if (cr > 0)
    {
        if (crlf > 0)
            eolStr += ", ";

        eolStr += wxString::Format("CR (Mac)=%d", cr);
    }
    if (lf > 0)
    {
        if ((crlf > 0) || (cr > 0))
        ...
        eolStr += wxString::Format("LF (Unix)=%d", lf);
    }
    if (eolStr.IsEmpty())
        eolStr = _("none");
                        ...
                        :wxDialog()
{
    m_notebook = notebook;
    m_listBox = NULL;
    wxCHECK_RET(m_notebook, "Invalid parent");
...
void wxSTEditorWindowsDialog::UpdateListBox()
{
    ...
    for (n = 0; n < count; n++)
    {
        ...
        wxString text = wxString::Format("%3d : ", n+1);
                         ...
                         :wxDialog()
{
    ...
    wxCHECK_RET(m_notebook || m_editor, "Invalid parent, must be a wxSTEditorNotebook or a wxSTEditor.");
...
void wxSTEditorBookmarkDialog::UpdateTreeCtrl()
{
    ...
    for (n = 0; n < count; n++)
    {
        ...
        while (line != -1)
        {
            if (!editorId)
            {
                wxString s(wxString::Format("%-5d : ", n+1) + editor->GetFileName().GetFullPath());
            ...
            wxString s(wxString::Format("%-5d : ", line+1) + editor->GetLineText(line));
            if (s.Length() > 100) s = s.Mid(0, 100) + "...";
                                           ...
                                           long& notebook_page, long& bookmark_line)
{
    ...
    if (m_treeCtrl->GetItemText(parentId).BeforeFirst(' ').Trim(false).ToLong(&notebook_page) &&
        m_treeCtrl->GetItemText(id).BeforeFirst(' ').Trim(false).ToLong(&bookmark_line) )
    {
...
void wxSTEditorInsertTextDialog::OnMenu(wxCommandEvent& event)
{
    ...
    switch (event.GetId())
    {
        case ID_STEDLG_INSERTMENU_TAB : c = "\t"; break;
        case ID_STEDLG_INSERTMENU_CR  : c = "\r"; break;
        case ID_STEDLG_INSERTMENU_LF  : c = "\n"; break;
        default : break;
    }
    ...
    if (c.Length())  // this must have been for the m_insertMenu
    {
        ...
        wxCHECK_RET(cBox, "Unexpected missing control");
...
{
    if (sm_splitBeforeArray.GetCount() == 0) sm_splitBeforeArray.Add("){}");
    if (sm_splitAfterArray.GetCount()  == 0) sm_splitAfterArray.Add("(,;");
    if (sm_preserveArray.GetCount()    == 0) sm_preserveArray.Add("\"\"");
    //if (sm_ignoreArray.GetCount()      == 0) sm_ignoreArray.Add("\"\"");
...
void wxSTEditorAboutDialog(wxWindow* parent)
{
...
#ifdef __WXDEBUG__
    if (!buildStr.IsEmpty()) buildStr += ", ";
    buildStr += "Debug"; // don't show release, that's assumed
#endif
    buildStr = " (" + buildStr + ")";

    msg.Printf( "Welcome to " STE_VERSION_STRING ".\n\n"
                "Using " wxVERSION_STRING ", http://www.wxwidgets.org\n"
                "and %s, http://www.scintilla.org\n"
                "\n"
                "Compiled on %s%s.",
   ...
   info.SetWebSite(STE_WEBSITE);
   info.SetLicense("wxWindows Licence\nhttp://www.wxwidgets.org/about/licence3.txt");
   info.AddDeveloper("John Labenski");
   info.AddDeveloper("Troels K");
   info.AddDeveloper("Otto Wyss");
   info.AddDeveloper("Antonio Cao");
