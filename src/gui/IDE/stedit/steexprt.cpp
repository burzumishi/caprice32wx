...
{
    wxCHECK_MSG(m_editor, false, "Invalid editor");
...
wxString wxSTEditorExporter::GetExtension(int file_format)
{
    switch (file_format)
    {
        ...
        case STE_EXPORT_HTMLCSS : return "html";
        case STE_EXPORT_PDF     : return "pdf";
        case STE_EXPORT_RTF     : return "rtf";
        case STE_EXPORT_TEX     : return "tex";
        case STE_EXPORT_XML     : return "xml";
        default : break;
    }
...
wxString wxSTEditorExporter::GetWildcards(int file_format)
{
    ...
    {
        case STE_EXPORT_HTML    :
        case STE_EXPORT_HTMLCSS : return "HTML (html,htm)|*.html;*.htm";
        case STE_EXPORT_PDF     : return "PDF (pdf)|*.pdf";
        case STE_EXPORT_RTF     : return "RTF (rtf)|*.rtf";
        case STE_EXPORT_TEX     : return "LaTex (tex)|*.tex";
        case STE_EXPORT_XML     : return "XML (xml)|*.xml";
        default : break;
    }
...
{
    wxCHECK_RET(styles.IsOk(), "Invalid styles");
    font       = wx2stc(styles.GetFaceName(ste_style));
    size       = styles.GetSize(ste_style);
    fore       = wx2stc(wxString::Format("#%06X", styles.GetForegroundColourInt(ste_style)));
    back       = wx2stc(wxString::Format("#%06X", styles.GetBackgroundColourInt(ste_style)));
...
bool wxSTEditorExporter::SaveToRTF(const wxFileName& saveName, int start, int end)
{
    ...
    FILE *fp = wxFopen(saveName.GetFullPath(), "wt");
    if (fp) {
...
bool wxSTEditorExporter::SaveToPDF(const wxFileName& saveName)
{
    ...
    FILE *fp = wxFopen(saveName.GetFullPath(), "wb");
    if (!fp)
    {
                          ...
                          StyleDefinition* sd, wxString &htmlString)
{
    ...
    if (style_n >= 0)
    {
        ...
        if ((old_style_n < 0) || (sd[old_style_n].fore != sd[style_n].fore))
            htmlString += wxString::Format("<FONT COLOR = \"%s\">", stc2wx(sd[style_n].fore.c_str()).wx_str());
...
{
    wxCHECK_MSG(m_editor, wxEmptyString, "Invalid editor");
    wxBusyCursor busy;
    ...
    wxString htmlString("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 Transitional//EN\">\n");
    htmlString << "<HTML>\n";

    // write the header
    htmlString << "<HEAD>\n";
    htmlString << "  <meta http-equiv=\"Content-Type\" content=\"text/html;charset=iso-8859-1\">\n";
    htmlString << "  <TITLE>") + fileName.GetFullPath() + "</TITLE>\n";
    htmlString << "</HEAD>\n";

    // write the body
    // FIXME: could also use <PRE WIDTH=80> for 80 column line widths
    htmlString << "<BODY><TT><PRE>\n";
    ...
    for (int n = from; n < to; n++)
    {
        ...
        switch (c)
        {
            case '\r' :
            {
                ...
                if ((n < (to - 1)) && (m_editor->GetCharAt(n+1) == '\n'))
                    break;
                // else fall through
            }
            ...
            case ' '  :
            {
                ...
                if (wysiwyg || (n == 0) || (m_editor->GetCharAt(n-1) == ' '))
                    htmlString << "&nbsp;";
                else
                    htmlString << c;
