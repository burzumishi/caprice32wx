...
wxString calltipWordCharacters("_0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ");
wxString autoCompleteStartCharacters(calltipWordCharacters);
wxString wordCharacters("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_#"); // FIXME add to langs
...
{
    "CRLF (Dos/MS Windows)",   // wxSTC_EOL_CRLF
    "CR (Mac)",                // wxSTC_EOL_CR
    "LF (Unix)"                // wxSTC_EOL_LF
};
                         ...
                         long style, const wxString& name )
{
    ...
    SetProperty("fold", "1"); // might as well turn them on even if unused
...
void wxSTEditor::RefEditor(wxSTEditor *origEditor)
{
    wxCHECK_RET(origEditor && (origEditor != this) &&
                (origEditor->GetRefData() != GetRefData()), "Invalid editor to ref");
...
{
    wxCHECK_MSG(str != NULL, false, "NULL input string to get clipboard data into.");
...
void wxSTEditor::PasteRectangular(const wxString& str, STE_TextPos pos)
{
    ...
    for ( ; tkz.HasMoreTokens(); line++)
    {
        ...
        if (line_end_pos < line_start_pos + line_pos)
            InsertText(line_end_pos, wxString(' ', line_start_pos + line_pos - line_end_pos));
...
wxString wxSTEditor::GetLineText(int line) const
{
    ...
    if (len > 0)
    {
        if (lineText[len-1] == '\n')
        {
            if ((len > 1) && (lineText[len-2] == '\r')) // remove \r\n for DOS
                return lineText.Mid(0, len-2);
            else
                return lineText.Mid(0, len-1);               // remove \n for Unix
        }
        else if (lineText[len-1] == '\r')               // remove \r for mac
                                       ...
                                       STE_TranslatePosType type)
{
    ...
    if (GetTabWidth() > 0) spaceString = wxString(' ', GetTabWidth());
    wxString findString(   !to_spaces ? spaceString : wxString("\t"));
    wxString replaceString( to_spaces ? spaceString : wxString("\t"));
                                 ...
                                 int top_line, int bottom_line)
{
    ...
    for (int n = top_line; n <= bottom_line; n++)
    {
        ...
        if (pos > line_end)
        {
            s = wxString(' ', size_t(pos - line_end)) + text;
            pos = line_end;
        }
                           ...
                           const wxString& ignoreAfterChars_)
{
    ...
    splitBefore.Replace(" ",  wxEmptyString, true);
    splitBefore.Replace("\t", wxEmptyString, true);
    splitBefore += " \t";

    wxString splitAfter(splitAfter_);
    splitAfter.Replace(" ",  wxEmptyString, true);
    splitAfter.Replace("\t", wxEmptyString, true);

    wxString ignoreAfterChars(ignoreAfterChars_);
    ignoreAfterChars.Replace(" ",  wxEmptyString, true);
    ignoreAfterChars.Replace("\t", wxEmptyString, true);
    ...
    for (wxStringTokenizer tkz(preserveChars, " ", wxTOKEN_STRTOK);
         tkz.HasMoreTokens();
         )
    {
        wxString token(tkz.GetNextToken());
    ...
    for (line = top_line; line <= bottom_line; line++)
    {
        ...
        for (int n = 0; n < len;  )
        {
            ...
            //wxPrintf("line %d col %d start %d len %d\n", line, col, match_start, match_len);
    ...
    for (line = top_line; line <= bottom_line; line++)
    {
        ...
        wxString newLine;
        if (max_start_pos > 0)
            newLine = wxString(' ', max_start_pos);
        ...
        for (col = 0; col < num_cols; col++)
        {
            ...
            if ((col < num_cols - 1) && (match_len < 1+maxLenArray[col]))
                newLine += wxString(' ', 1+maxLenArray[col]-match_len);
...
#define STE_VERSION_STRING_SVN STE_VERSION_STRING // " svn 2920"
    ...
    }
    {
        ...
        if (ok)
        {
            ...
            if (want_lang && !found_lang)
            {
                ...
                if (found_lang)
                {
                    is_html = (0 == GetEditorLangs().GetName(GetLanguageId()).CmpNoCase("html"));
                    is_xml  = (0 == GetEditorLangs().GetName(GetLanguageId()).CmpNoCase("xml"));
                }
            }
            if ((want_lang && !found_lang) || ( (is_html || is_xml) && (encoding == wxTextEncoding::Ascii)) )
            {
                ...
                if (want_lang && !found_lang)
                {
                    ...
                    {
                        found_lang = is_xml = SetLanguage(wxFileName(wxEmptyString, fileName.GetName(), "xml"));
                    }
                }
...
{
    wxCHECK_MSG(GetFindReplaceData(), wxEmptyString, "Invalid find/replace data");
...
{
    wxCHECK_MSG(GetFindReplaceData(), 0, "Invalid find/replace data");
...
void wxSTEditor::ShowFindReplaceDialog(bool find)
{
    ...
    wxCHECK_RET(steFindReplaceData != NULL, "Invalid find/replace data");
...
{
    wxCHECK_RET(GetFindReplaceData(), "Invalid find/replace data");
...
wxString wxSTEditor::EliminateDuplicateWords(const wxString& words0) const {
    ...
    wxStringTokenizer tokenizer(words0, " ");
...
void wxSTEditor::UpdateItems(wxMenu *menu, wxMenuBar *menuBar, wxToolBar *toolBar)
{
    ...
    STE_MM::DoSetTextItem(menu, menuBar, ID_STE_PREF_EOL_MODE,     _("&EOL Mode (")+wxSTC_EOL_Strings[GetEOLMode()].BeforeFirst(' ')+")...");
    ...
    switch (win_id)
    {
        ...
        case ID_STE_LINES_SPLIT :
        {
            ...
            if (win_id == ID_STE_LINES_JOIN)
                ...
                LinesSplit(TextWidth(wxSTC_STYLE_DEFAULT, wxString('W', GetEdgeColumn())));
            return true;
        }
        ...
        case ID_STE_BOOKMARKS :
        {
            ...
            while (line >= 0)
            {
                ...
                wxString s(wxString::Format("%-5d : ", line+1) + GetLineText(line));
            ...
            {
                long l = 0;
                if (choice.BeforeFirst(' ').Trim(false).ToLong(&l))
                    GotoLine(l-1);
            }
            return true;
        }
...
static void wxSTEditor_SplitLines(wxSTEditor* editor, int pos, int line_n) // FIXME verify wrapping code
{
    ...
    //wxPrintf("wxSTEditor_SplitLines\n");
        ...
        for (n = edge_col-1; n >= 0; n--)
        {
            if ((s[n] == ' ') || (s[n] == '\t'))
            {
                ...
                if (line_n + 1 < line_count)
                {
                    ...
                    {
                        editor->InsertText(editor->PositionFromLine(line_n+1), "\n");
                    }
                }
