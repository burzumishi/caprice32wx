...
bool wxLocaleHelper::SingleChoice(const wxArrayInt& languages, wxLanguage* selected_language)
{
    ...
    int index = wxGetSingleChoiceIndex("Language", wxMessageBoxCaption, languageNames);
...
static bool wxMenuItem_SetAccelText(wxMenuItem* item, const wxString& accel, bool append = true)
{
   ...
   item->SetItemLabel(wxString::Format("%s%s%s",
      str.wx_str(),
      ch_sep.wx_str(),
      accel.wx_str()));
   return true;
...
static wxString wxGetAccelText(int flags, int keyCode)
{
   ...
   switch (keyCode)
   {
      ...
      default:
      {
         ...
         {
            str += wxString::Format("F%d", keyCode - WXK_F1 + 1);
         }
         else
         {
            wxASSERT(keyCode >= ' ');
...
wxString wxToolBarTool_MakeShortHelp(const wxString& rstr, const wxArrayAcceleratorEntry& accel, int id)
{
   ...
   if (accel.GetCount() && !str.IsEmpty())
   {
      ...
      for (i = 0; i < count; i++)
      {
         ...
         {
            if (!strAccel.IsEmpty()) strAccel+=ACCELSTR_SEP;
            strAccel+=wxGetAccelText(element);
         }
      }
      if (!strAccel.IsEmpty())
      {
         str += wxString::Format(" (%s)", strAccel.wx_str());
      }
   }
   return str;
}
...
{
    wxCHECK_MSG(clip_type != CLIPBOARD_BOTH, false, "Getting values from both clipboards is not supported");
