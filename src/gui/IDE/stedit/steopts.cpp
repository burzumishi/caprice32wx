...
wxString STE_DefaultFileName( "untitled.txt" );
wxString STE_DefaultFileExtensions(
                                    "All Files|" wxALL_FILES_PATTERN "|"
                                #if !STE_FILEOPENEXTRA
                                    "UTF8 Text Files|" wxALL_FILES_PATTERN "|"
                                    "Unicode Text Files|" wxALL_FILES_PATTERN "|"
                                #ifdef __WXMSW__
                                    "OEM Text Files|" wxALL_FILES_PATTERN "|"
                                #endif
                                #endif
                                    "Text Files (txt text)|*.txt;*.text" "|"
                                    "C/C++ Files (c cpp cxx)|*.c;*.cpp;*.cxx" "|"
                                    "H Files (h)|*.h" "|"
...
public:
                                    ...
                                    m_displayPathSeparator(wxPATH_NATIVE)
    {
        ...
        m_optionNames.Add("STE_OPTION_EDITOR");
        m_optionNames.Add("STE_OPTION_SPLITTER");
        m_optionNames.Add("STE_OPTION_NOTEBOOK");
        m_optionNames.Add("STE_OPTION_FRAME");
        m_optionNames.Add("STE_OPTION_CONFIG");
        m_optionNames.Add("STE_OPTION_FINDREPLACE");
        m_optionNames.Add("STE_OPTION_DEFAULT_FILENAME");
        m_optionNames.Add("STE_OPTION_DEFAULT_FILEPATH");
        m_optionNames.Add("STE_OPTION_DEFAULT_FILEEXTS");
        m_optionNames.Add("STE_OPTION_CFGPATH_BASE");
        m_optionNames.Add("STE_OPTION_CFGPATH_PREFS");
        m_optionNames.Add("STE_OPTION_CFGPATH_STYLES");
        m_optionNames.Add("STE_OPTION_CFGPATH_LANGS");
        m_optionNames.Add("STE_OPTION_CFGPATH_FRAME");
        m_optionNames.Add("STE_OPTION_CFGPATH_FILEHISTORY");
        m_optionNames.Add("STE_OPTION_CFGPATH_FINDREPLACE");
                                     ...
                                     const wxString& defaultFileExt )
{
    ...
    SetOption(STE_OPTION_CFGPATH_BASE,        "/wxSTEditor");
    SetOption(STE_OPTION_CFGPATH_PREFS,       "Preferences");
    SetOption(STE_OPTION_CFGPATH_STYLES,      "Styles");
    SetOption(STE_OPTION_CFGPATH_LANGS,       "Languages");
    SetOption(STE_OPTION_CFGPATH_FRAME,       "Frame");
    SetOption(STE_OPTION_CFGPATH_FILEHISTORY, "RecentFiles");
    SetOption(STE_OPTION_CFGPATH_FINDREPLACE, "FindReplace");
...
{
    wxCHECK_RET(STEO_REFDATA, "invalid wxSTEditorOptions");
    STEO_REFDATA->SetClientObject(data);
}
wxClientData *wxSTEditorOptions::GetClientObject() const
{
    wxCHECK_MSG(STEO_REFDATA, NULL, "invalid wxSTEditorOptions");
...
wxString wxSTEditorOptions::GetConfigPath(size_t path_option_n) const
{
    ...
    if (optionPath.Length() && (optionPath[0] == '/'))
        return optionPath;
...
{
    if (add_sep && (!path.Length() || (path.Last() != '/')))
        return path + "/";
    if (!add_sep && path.Length() && (path.Last() == '/'))
...
void wxSTEditorOptions::LoadFileConfig( wxConfigBase &config)
{
    ...
    wxString value, key = configPath+"/LastDir";
...
void wxSTEditorOptions::SaveFileConfig(wxConfigBase &config)
{
    ...
    wxString configPath = FixConfigPath(GetConfigPath(STE_OPTION_CFGPATH_FILEHISTORY), false);
    config.Write(configPath+"/LastDir", GetDefaultFilePath());
