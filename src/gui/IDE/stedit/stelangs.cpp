...
{
    wxCHECK_RET(other.IsOk(), "Langs not created");
    if (!IsOk()) Create();
...
{
    wxCHECK_RET(IsOk(), "Langs not created");
...
{
    wxCHECK_MSG(IsOk() && langs.IsOk(), false, "Langs not created");
...
{
    wxCHECK_MSG(IsOk() && lang, -1, "Langs not created");
...
{
    wxCHECK_MSG(IsOk(), 0, "Langs not created");
...
int wxSTEditorLangs::FindLanguageByFilename(const wxFileName& fileName_) const
{
    ...
    wxCHECK_MSG(IsOk(), fallback, "Langs not created");
    ...
    for (lang_n = 0; lang_n < lang_count; lang_n++)
    {
        ...
        for (wxStringTokenizer tokenizer(filePattern, ";");
             ...
             )
        {
            ...
            if ((wildToken == "*") || (wildToken == "*.*"))
            {
                fallback = (int)lang_n; // try for better match
            }
            else if ( ((wildExt  == "*") || (wildExt  == ext )) &&
...
{
    wxCHECK_MSG(IsOk(), NULL, "Langs not created");
    wxCHECK_MSG(lang_n<GetCount(), NULL, "Invalid language index");
...
wxString wxSTEditorLangs::GetFileFilter(size_t lang_n) const
{
    ...
    return GetName(lang_n) + " (" + filePattern + ")|" + filePattern;
}
...
{
    wxCHECK_MSG(style_n<GetStyleCount(lang_n), 0, "Invalid language style type");
...
{
    wxCHECK_MSG(style_n<GetStyleCount(lang_n), -1, "Invalid language style type");
...
{
    wxCHECK_MSG(style_n<GetStyleCount(lang_n), wxEmptyString, "Invalid language style type");
...
{
    wxCHECK_MSG(word_n<GetKeyWordsCount(lang_n), wxEmptyString, "Invalid language keyword");
    wxString words;
...
{
    wxCHECK_MSG(HasLanguage(lang_n), 0, "Invalid language");
...
{
    wxCHECK_RET(GetLanguage(lang_n), "Langs not created");
    wxCHECK_RET(lang_n<GetCount(), "Invalid language info item");
...
{
    wxCHECK_RET(GetLanguage(lang_n), "Langs not created");
    wxCHECK_RET(style_n<GetStyleCount(lang_n), "Invalid language style type");
...
void wxSTEditorLangs::SetUserSTEStyle(size_t lang_n, size_t style_n, int ste_style)
{
    ...
    wxString strStyle = wxString::Format("%d", (int)ste_style);
...
void wxSTEditorLangs::SetUserKeyWords(size_t lang_n, size_t word_n, const wxString& words)
{
    wxCHECK_RET(GetLanguage(lang_n), "Langs not created");
    wxCHECK_RET(lang_n<GetCount(), "Invalid language info item");
    wxCHECK_RET(word_n<GetKeyWordsCount(lang_n), "Invalid language keyword");
                                  ...
                                  const wxString &configPath )
{
    ...
    for (size_t lang_n = 0; lang_n < GetCount(); lang_n++)
    {
        ...
        {
            keyName = keyBase + wxString::Format("/Style_%d", (int)style_n);
            long l_value = 0;
        ...
        {
            keyName = keyBase + wxString::Format("/Keyword_%d", (int)word_n);
            if (config.Read(keyName, &value))
                                  ...
                                  int WXUNUSED(flags) ) const
{
    ...
    for (size_t lang_n = 0; lang_n < GetCount(); lang_n++)
    {
        ...
        for (size_t style_n = 0; style_n < GetStyleCount(lang_n); style_n++)
        {
            keyName        = keyBase + wxString::Format("/Style_%d", (int)style_n);
            int user_style = GetUserSTEStyle(lang_n, style_n);
            value          = wxString::Format("%d", user_style);
...
void wxSTEditorLangs::UpdateEditor( wxSTEditor *editor )
{
    ...
    wxCHECK_RET(HasLanguage(lang_n), "Invalid language id");
...
void LangConfig()
{
    wxFileConfig c("wxStEditLangs", "wxWidgets",
                   "stelangsfconfig.txt", wxEmptyString,
                   wxCONFIG_USE_RELATIVE_PATH);

    wxFileConfig *config = &c;

    wxString configRoot = "wxSTEditor";
    wxString configGroup = "/Languages";
    wxString key = configRoot + configGroup + "/";
    ...
    {
        // wxString::Format("%d ", n) +
        wxString keyName = key + langs.GetName(n) + "/";

        config->Write(keyName + "File_Patterns", langs.GetFilePattern(n));
        config->Write(keyName + "File_Filters",  langs.GetFileFilter(n));
        config->Write(keyName + "Lexer",         langs.GetLexer(n));

        //config->Write(keyName + "Style_Count",  langs.GetStyleCount(n));
        //for (i = 0; i < langs.GetStyleCount(n); i++)
        //    config->Write(keyName + wxString::Format("S%d", i), langs.GetSTEStyle(n, i));

        //config->Write(keyName + "Keyword_Count",  langs.GetKeyWordsCount(n));
        for (i = 0; i < langs.GetKeyWordsCount(n); i++)
            config->Write(keyName + wxString::Format("Keyword%d", (int)i), langs.GetKeyWords(n, i));

        if (langs.GetBlockStart(n).Length())
            config->Write(keyName + "BlockStart",         langs.GetBlockStart(n));
        if (langs.GetBlockEnd(n).Length())
            config->Write(keyName + "BlockEnd",           langs.GetBlockEnd(n));

        if (langs.GetPreprocessorSymbol(n).Length())
            config->Write(keyName + "PreprocessorSymbol", langs.GetPreprocessorSymbol(n));
        if (langs.GetPreprocessorStart(n).Length())
            config->Write(keyName + "PreprocessorStart",  langs.GetPreprocessorStart(n));
        if (langs.GetPreprocessorMid(n).Length())
            config->Write(keyName + "PreprocessorMid",    langs.GetPreprocessorMid(n));
        if (langs.GetPreprocessorEnd(n).Length())
            config->Write(keyName + "PreprocessorEnd",    langs.GetPreprocessorEnd(n));

        if (langs.HasComments(n))
        {
            config->Write(keyName + "CommentBlockAtLineStart",    langs.GetCommentBlockAtLineStart(n));
            if (langs.GetCommentBlock(n).Length())
                config->Write(keyName + "CommentBlock",    langs.GetCommentBlock(n));
            if (langs.GetCommentBoxStart(n).Length())
                config->Write(keyName + "CommentBoxStart",    langs.GetCommentBoxStart(n));
            if (langs.GetCommentBoxMiddle(n).Length())
                config->Write(keyName + "CommentBoxMiddle",    langs.GetCommentBoxMiddle(n));
            if (langs.GetCommentBoxEnd(n).Length())
                config->Write(keyName + "CommentBoxEnd",    langs.GetCommentBoxEnd(n));
            if (langs.GetCommentStreamStart(n).Length())
                config->Write(keyName + "CommentStreamStart",    langs.GetCommentStreamStart(n));
            if (langs.GetCommentStreamEnd(n).Length())
                config->Write(keyName + "CommentStreamEnd",    langs.GetCommentStreamEnd(n));
        }

        config->Write(keyName + "Folds",              langs.GetFolds(n));
        config->Write(keyName + "Flags",              langs.GetFlags(n));
    }
