...
{
    int ret = wxStrcmp(first.BeforeLast('='), second.BeforeLast('='));
    if (ret == 0)
    {
        // same names, keep the same order
        long f = 0, s = 0;
        wxCHECK_MSG(first.AfterLast('=').ToLong(&f), 0, "Invalid first page name for sorting");
        wxCHECK_MSG(second.AfterLast('=').ToLong(&s), 0, "Invalid second page name for sorting");
        ret = f > s ? 1 : -1;
    }
...
void wxSTEditorNotebook::OnPageChanged(wxNotebookEvent &event)
{
    ...
    if (event.GetString() == "wxSTEditorNotebook Page Change")
    {
...
wxSTEditorSplitter *wxSTEditorNotebook::CreateSplitter(wxWindowID id)
{
        ...
        (wxDynamicCast(event.GetEventObject(), wxSTEditorSplitter) != NULL))
    {
        ...
        {
            wxFAIL_MSG("Incorrect parent for wxSTEditorSplitter, should be wxSTEditorNotebook");
            return NULL;
        }
    }
    else
                                                             ...
                                                             const wxString& title, bool bSelect)
{
    ...
    wxCHECK_MSG(splitter, NULL, "Invalid splitter");
                                              ...
                                              bool bSelect)
{
    wxCHECK_MSG(splitter && (splitter->GetParent() == this), false,
                "Invalid wxSTEditorSplitter or parent");
    ...
    if (nPage < 0) // they want to insert it anywhere
    {
        ...
        {
            wxArrayString names;
            names.Add(title+"=999999"); // insert after any other pages with same name

            for (size_t n = 0; n < n_pages; n++)
            {
                wxString name(GetPageText(n));
                if ((name.Length() > 0) && (name[0u] == '*'))
                    name = name.Mid(1);

                names.Add(name + wxString::Format("=%d", (int)n));
            }

            names.Sort(STN_SortNameCompareFunction);
            nPage = names.Index(title+"=999999");
        }
        else
            nPage = (int)n_pages;
    }
...
void wxSTEditorNotebook::SortTabs(int style)
{
    ...
    if (STE_HASBIT(style, STN_ALPHABETICAL_TABS))
    {
        ...
        {
            long old_page = 0;
            names[n].AfterLast('=').ToLong(&old_page);
        ...
        if (sel_changed)
        {
            wxNotebookEvent noteEvent(wxEVT_COMMAND_NOTEBOOK_PAGE_CHANGED, GetId(),
                                    new_sel, new_sel);
            noteEvent.SetString("wxSTEditorNotebook Page Change");
...
{
    wxCHECK_MSG((n >= 0) && (n < (int)GetPageCount()), false, "Invalid page");
    wxSTEditor *editor = GetEditor(n);
        ...
        case wxID_SAVEAS:
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    wxCHECK_MSG(splitter, true, "Invalid splitter");
                    wxSTEditor *newEditor = splitter->GetEditor();
                    wxCHECK_MSG(newEditor, true, "Invalid splitter editor");
                                   ...
                                   const wxString& encoding_ref)
{
    ...
    if (ok)
    {
        ...
        else if ( (GetEditor() == NULL) || GetEditor()->IsModified() || GetEditor()->IsFileFromDisk()) // non-empty editor?
        {
            // new splitter+editor
            wxSTEditorSplitter *splitter = CreateSplitter(wxID_ANY);
            wxCHECK_MSG(splitter, false, "Invalid splitter");
                                    ...
                                    const wxString &extensions_)
{
    ...
    wxProgressDialog progDlg(_("Loading files..."),
                             wxString('_', max_filename_len + 10), // +10 for file count
    ...
    for (n = 0; n < count; n++)
    {
        ...
        if (fileName.Length() > max_filename_len)
            progressFileName = fileName.Mid(0, max_filename_len/2-2) + " ... " + fileName.Mid(fileName.Length()-max_filename_len/2+2);
        else
            progressFileName = fileName;

        if (!progDlg.Update((int)n, wxString::Format("%d/%d : ", (int)n+1, (int)count) + progressFileName))
            break;
...
/*
    ...
    {
        label = wxString::Format("%2d : %s", n+1, GetPageText(n).wx_str());
        if (menu->GetLabel(startID+n) != label)
            menu->SetLabel(startID+n, label);
    }
    // append new pages
    for (n=item_count; n<page_count; n++)
        menu->AppendRadioItem(startID+n, wxString::Format("%2d : %s", n+1, GetPageText(n).wx_str()));
*/
/*
...
// ==== check items do not
    ...
    for (n = 0; n < item_count; n++)
    {
        label = wxString::Format("%2d : %s", (int)n+1, GetPageText(n).wx_str());
    ...
    // append new pages
    for (n = item_count; n < page_count; n++)
        menu->AppendCheckItem(int(startID+n), wxString::Format("%2d : %s", (int)n+1, GetPageText(n).wx_str()));
...
/*
    ...
    for (n = 0; n < item_count; n++)
        menu->Delete(startID+n);
    for (n = 0; n < page_count; n++)
        menu->AppendCheckItem(startID+n, wxString::Format("%2d : %s", n+1, GetPageText(n).wx_str()));
*/
