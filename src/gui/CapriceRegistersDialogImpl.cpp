...
void CapriceRegistersDialogImpl::OnInitR( wxInitDialogEvent& event )
{
	...
	for(unsigned int i = 0; i < t; i++) {
		...
		if (l.val == NULL)
			text.Printf("Read side %d, track %d, NOT FOUND", l.s, l.t);
		else
			text.Printf("Read side %d, track %d, CHRN %.8X ST %.8X", l.s, l.t, *(int*)l.val->CHRN, *(int*)l.val->flags);
...
//     CRTC Register
	...
	{
		a_track->SetValue(wxString::Format("%d",emulator->GetFDC()->GetDriveA().current_track));
		a_sector->SetValue(wxString::Format("%d",emulator->GetFDC()->GetDriveA().current_sector));
		b_track->SetValue(wxString::Format("%d",emulator->GetFDC()->GetDriveB().current_track));
		b_sector->SetValue(wxString::Format("%d",emulator->GetFDC()->GetDriveB().current_sector));

		t_st0->SetValue(wxString::Format("%x",emulator->GetFDC()->getST(0)));
		t_st1->SetValue(wxString::Format("%x",emulator->GetFDC()->getST(1)));
		t_st2->SetValue(wxString::Format("%x",emulator->GetFDC()->getST(2)));
		t_st3->SetValue(wxString::Format("%x",emulator->GetFDC()->getST(3)));
